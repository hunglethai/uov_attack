...
# Generate and check for full-rank matrices i.e. public keys M[i]
M = regenerate_until_full_rank(F, m, n, O)

# Compute symplectic basis L_i of M[i]
...
# L_submatrix = compute_isotropic_subspace_basis(F,L,M,m,n,False) # False if not Lagrangian, True otherwise
...
P = Q.inverse()*R
print("Is P*M[1]*P^t== M[0] ? ", P*M[1]*P.transpose() == M[0])

# Invariant subspace of P
basis = compute_invariant_subspace(F,P)
print(len(basis))
filtered_basis = []
for i, v in enumerate(basis):
    ...
    for j, u in enumerate(basis):
        if i != j:
            # Check the condition v^T * M[0] * u == 0 and v^T * M[1] * u == 0
            condition1 = v * M[0] * u.column() == 0
            condition2 = v * M[1] * u.column() == 0
            if not (condition1 and condition2):
                is_valid = False
                break
    
    if is_valid:
        filtered_basis.append(v)
print(filtered_basis)

"""
# Check if rows of O_I.transpose() is generated by symplectic basis of M0

matrices_list = L_submatrix[0] # isotropic basis of M[0]
# Compute T (m*m) such that T*L_0 = L_1*P with L_i of size m *n and P size n*n
T, L_0_0, L_0_1 = compute_transformation_T(matrices_list, P)
